...
import { PlaySquare, ArrowRight, Zap, Brain, Clock, BarChart2, Link2, Search, CheckCircle2, ChevronRight, Play, Sparkles, MessageCircleQuestion, ChevronDown, Rocket, Shield, Globe, Cpu, Database, Layers, LayoutGrid, Loader2 } from "lucide-react";
...
function AnimatedTerminal() {
  ...
  return (
    <div className="bg-[#0b0906] border border-white/10 rounded-3xl p-8 relative shadow-2xl min-h-[300px] flex flex-col justify-center">
      ...
      <div className="space-y-4 font-mono text-sm leading-relaxed">
        {steps.map((s, i) => (
          ...
          >
            <span className={cn("shrink-0 font-bold", step > i ? "text-green-400" : "text-amber-400")}>{">"}</span>
            <span className={cn("flex items-center gap-2", step > i ? "text-green-400" : "text-amber-400")}>
              {s.text}
              {step === i && <Loader2 className="w-3 h-3 animate-spin text-amber-400" />}
    ...
    >
      ...
      >
        <span className="text-lg font-semibold font-display flex items-center gap-3">
          <MessageCircleQuestion className="w-[18px] h-[18px] sm:w-5 sm:h-5 text-amber-400 shrink-0 stroke-[2.25]" />
          {question}
        </span>
        <motion.div
...
function MatchRow({ time, snippet, confidence }: { time: string, snippet: string, confidence: number }) {
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-3 text-[11px]">
        <span className="flex items-center gap-2 min-w-0">
          <span className="shrink-0 px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 font-mono font-bold">{time}</span>
          <span className="text-gray-400 truncate">{snippet}</span>
        </span>
        <span className="shrink-0 text-amber-400 font-mono font-bold">{confidence}%</span>
      </div>
      <div className="h-1.5 w-full bg-amber-500/10 rounded-full overflow-hidden relative">
        <motion.div
          initial={{ width: 0 }}
        ...
        >
          <div className="absolute inset-y-0 right-0 w-4 bg-amber-400 blur-md opacity-50" />
        </motion.div>
      </div>
    </div>
  );
...
export function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0906] text-white selection:bg-amber-500/30 overflow-x-hidden relative">
      {/* Background Effects */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] rounded-full bg-amber-600/10 blur-[120px]" />
        <div className="absolute bottom-[-20%] right-[-10%] w-[50%] h-[50%] rounded-full bg-rose-600/10 blur-[120px]" />
      </div>
      ...
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-4xl px-4">
        <motion.div
          ...
          className="bg-[#13100b]/70 backdrop-blur-2xl border border-white/10 rounded-full px-6 h-14 flex items-center justify-between shadow-[0_20px_40px_rgba(0,0,0,0.5)]"
        >
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-1.5 px-3 h-9 rounded-full bg-green-500/10 border border-green-500/20 text-green-400 text-[10px] font-bold uppercase tracking-widest">
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-amber-400/0 via-amber-400/20 to-rose-400/0 opacity-0 group-hover:opacity-100 translate-x-[-100%] group-hover:translate-x-[100%] transition-all duration-[1s] ease-in-out pointer-events-none" />
            </Link>
          </div>
        </motion.div>
      </nav>
      ...
      <main className="pt-32 pb-24 relative z-10">
        ...
        >
          ...
          <motion.h1 variants={fadeIn} className="text-5xl md:text-6xl font-bold tracking-tight mb-8 leading-[1.05] font-display">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-orange-400 to-rose-400 font-serif font-bold italic">at the speed of thought</span>
          </motion.h1>
          <motion.p variants={fadeIn} className="text-lg md:text-xl text-gray-400 mb-10 max-w-2xl mx-auto leading-relaxed">
            Stop watching hours of filler. ClipIQ uses advanced RAG to extract precise insights and pinpoint exact timestamps in seconds.
          </motion.p>
          <motion.div variants={fadeIn} className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            >
              <div className="flex items-center gap-2 relative z-10">
                <div className="w-9 h-9 rounded-full bg-amber-50/80 flex items-center justify-center group-hover:bg-amber-600 transition-all duration-500 shadow-sm border border-amber-100/20">
                  <Sparkles className="w-4.5 h-4.5 text-amber-600 group-hover:text-white transition-colors" />
                </div>
                <span className="uppercase tracking-[0.05em] font-display text-[12px] font-bold">Try ClipIQ Free</span>
                <ArrowRight className="w-4 h-4 transition-all duration-300 group-hover:translate-x-1 group-hover:text-amber-600" />
              </div>
              ...
              {/* Premium Inner Ring */}
              <div className="absolute inset-0 rounded-full border border-black/[0.03] group-hover:border-amber-200/20 transition-colors" />
            </Link>
            <a
              href="#how-it-works"
            ...
            >
              <div className="flex items-center gap-2.5">
                <Play className="w-3.5 h-3.5 transition-transform group-hover:scale-110 group-hover:text-amber-400" />
          ...
          <div className="mt-32 relative">
            ...
            <motion.div
              ...
              className="relative rounded-[2rem] overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(217,119,6,0.15)] bg-[#13100b]"
            >
              <div className="absolute inset-0 bg-gradient-to-t from-[#0b0906] via-transparent to-transparent z-10 pointer-events-none"></div>
            ...
            >
              {[
                { stat: "1M+", label: "Videos Indexed", color: "from-yellow-500/20 to-yellow-500/5", textColor: "text-yellow-400" },
                { stat: "99%", label: "Timestamp Accuracy", color: "from-rose-500/20 to-rose-500/5", textColor: "text-rose-400" },
                { stat: "12s", label: "Avg. Processing Time", color: "from-red-500/20 to-red-500/5", textColor: "text-red-400" },
        ...
        <section className="py-24 relative">
          <div className="max-w-7xl mx-auto px-6 mb-12 text-center">
            <p className="text-[10px] font-bold text-amber-500/60 uppercase tracking-[0.4em] mb-2">The Stack</p>
            ...
            <div className="flex gap-20 animate-marquee whitespace-nowrap py-4">
              {[
                ...
                <div key={i} className="flex items-center gap-6 group cursor-default">
                  <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center group-hover:bg-amber-500/10 group-hover:border-amber-500/20 transition-all duration-500">
                    <tech.icon className="w-6 h-6 text-white/20 group-hover:text-amber-400 group-hover:scale-110 transition-all duration-500" />
                  </div>
        ...
        <section id="features" className="max-w-7xl mx-auto px-6 mb-32 pt-20">
          ...
          >
            <h2 className="text-5xl md:text-6xl font-bold mb-6 font-display">Deep reasoning, <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-rose-400 to-red-400 font-serif font-bold italic">zero filler.</span></h2>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: Brain, title: "Single-Video RAG", desc: "Maps every concept to precise timestamps. Ask specifics, get direct video jump-points.", color: "yellow" },
              { icon: MessageCircleQuestion, title: "Interactive Chat", desc: "Ask follow-up questions about the video and get grounded answers with clickable timestamp sources.", color: "purple" },
              ...
              <motion.div
                ...
                className={cn(
                  ...
                  feature.color === "yellow" ? "hover:shadow-yellow-500/10" : "",
                  feature.color === "purple" ? "hover:shadow-rose-500/10" : "",
                  feature.color === "pink" ? "hover:shadow-red-500/10" : "",
                  feature.color === "amber" ? "hover:shadow-amber-500/10" : "",
                  feature.color === "indigo" ? "hover:shadow-orange-500/10" : "",
              ...
              >
                <div className={cn(
                  ...
                  feature.color === "purple" ? "bg-rose-500/20 text-rose-400" : "",
                  feature.color === "pink" ? "bg-red-500/20 text-red-400" : "",
                  feature.color === "amber" ? "bg-amber-500/20 text-amber-400" : "",
                  feature.color === "indigo" ? "bg-orange-500/20 text-orange-400" : "",
        ...
        <section id="how-it-works" className="max-w-7xl mx-auto px-6 mb-32 pt-20">
          <motion.div
            ...
            className="bg-gradient-to-b from-[#13100b] to-[#0b0906] border border-white/5 rounded-3xl md:rounded-[3rem] p-6 sm:p-10 md:p-20 relative overflow-hidden shadow-2xl"
          >
            <div className="absolute top-0 right-0 w-1/2 h-full bg-amber-500/5 blur-[150px] pointer-events-none"></div>

            <div className="grid lg:grid-cols-2 gap-16 items-center relative z-10">
              <div>
                <h2 className="text-5xl md:text-6xl font-bold mb-6 font-display">From URL to insight in <br /><span className="text-amber-400 font-serif font-bold italic">three steps</span></h2>
                ...
                <div className="space-y-8">
                  {[
                    { icon: Link2, title: "Provide Context", desc: "Paste a YouTube URL. We fetch the metadata, transcript, and channel authority." },
                    ...
                    { icon: CheckCircle2, title: "Actionable Output", desc: "Get a timestamped summary and direct answers to your specific questions in chat." },
                  ].map((step, i) => (
                    <motion.div
                      key={i}
              ...
              <motion.div
                ...
                className="relative"
              >
                <div className="absolute inset-0 bg-gradient-to-tr from-amber-500/20 to-rose-500/20 rounded-3xl blur-2xl"></div>
                <AnimatedTerminal />
              </motion.div>
            </div>
          </motion.div>
        ...
        <section className="max-w-7xl mx-auto px-6 mb-32">
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            ...
            >
              ...
              <h2 className="text-5xl md:text-6xl font-bold font-display leading-[1.1]">The world's first <br /> <span className="text-amber-400 font-serif font-bold italic">temporal search</span></h2>
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                <div className="p-6 bg-white/5 rounded-3xl border border-white/5">
                  <Globe className="w-6 h-6 text-amber-400 mb-4" />
            ...
            <motion.div
              ...
              className="relative"
            >
              <div className="absolute inset-0 bg-amber-500/20 blur-[100px] pointer-events-none rounded-full"></div>
              <div className="bg-gradient-to-br from-white/10 to-white/5 border border-white/10 rounded-[3rem] p-8 backdrop-blur-3xl shadow-3xl">
                <div className="space-y-6">
                  <div className="flex items-center justify-between border-b border-white/5 pb-4">
                    <div className="font-bold">Semantic Match</div>
                    <div className="text-amber-400 text-xs font-mono">CLIPIQ_ENGINE: 2.1</div>
                  </div>
                  <p className="text-[11px] text-gray-500 font-mono">Query: "How do I handle errors here?"</p>
                  {[
                    { time: "14:22", snippet: "Wrapping the call in a try/except block...", confidence: 96 },
                    { time: "31:05", snippet: "Custom exception classes for clarity...", confidence: 88 },
                    { time: "42:50", snippet: "Logging errors before re-raising...", confidence: 81 },
                  ].map((match, i) => (
                    <MatchRow key={i} {...match} />
                  ))}
                  <div className="pt-4 text-xs text-gray-500 leading-relaxed italic border-t border-white/5">
                    "ClipIQ Result: Grounded to the exact second — every answer cites a real timestamp, not a guess."
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        <section id="faqs" className="max-w-4xl mx-auto px-6 mb-32">
          ...
          <div className="space-y-4">
            {[
              ...
              { q: "Does it support playlist analysis?", a: "Currently, we focus on deep-dives for a single video at a time. Multi-video playlist summarization is coming in a future update!" }
            ].map((faq, i) => (
            ...
            <Link to="/dashboard" className="inline-flex items-center justify-center">
              <button className="relative group px-10 py-5 rounded-full font-bold text-lg bg-white text-black overflow-hidden transition-all duration-500 hover:scale-[1.05] active:scale-95 shadow-[0_20px_50px_-10px_rgba(255,255,255,0.15)]">
                <div className="flex items-center gap-3 relative z-10">
                  <div className="w-10 h-10 rounded-full bg-amber-50 flex items-center justify-center group-hover:bg-amber-600 transition-colors duration-500">
                    <Rocket className="w-5 h-5 text-amber-600 group-hover:text-white transition-colors" />
                  </div>
      ...
      <footer className="border-t border-white/5 pt-24 pb-12 bg-[#0b0906] relative z-10 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 gap-12 mb-20">
          <div>
            <div className="flex items-center gap-2 mb-6">
              <div className="w-8 h-8 bg-gradient-to-br from-amber-500 to-amber-700 rounded-lg flex items-center justify-center shadow-lg shadow-amber-500/20">
                <PlaySquare className="w-5 h-5 text-white" />
              </div>
