...
import { Clock, Trash2, Edit2, Check, X, ArrowUpRight, Search, Zap } from "lucide-react";
...
export function History() {
    ...
    useEffect(() => {
        setItems(getHistory().filter((item) => item.type === "Summary"));
    }, []);
    ...
    const handleDelete = async (id: string, e: React.MouseEvent) => {
        ...
        const updated = getHistory().filter((entry) => entry.type === "Summary");
        setItems(updated);
    ...
    const saveEdit = (id: string, e: React.MouseEvent) => {
        ...
        if (editValue.trim()) {
            ...
            setItems(getHistory().filter((entry) => entry.type === "Summary"));
    ...
    return (
        <div className="w-full bg-[#0b0906] selection:bg-amber-500/30 relative pb-[72px] overflow-x-hidden">
            <div className="p-4 md:p-6 lg:p-10 pt-4 md:pt-6 pb-40">
                ...
                >
                                ...
                                <div className="relative">
                                    {!showConfirmClear ? (
                                        ...
                                        <motion.div
                                            ...
                                            className="flex items-center gap-2 bg-[#241d15] p-1 rounded-xl md:rounded-2xl border border-red-500/30 shadow-2xl"
                                        >
                                            <button
                                                onClick={handleClear}
                    ...
                    {/* Main List */}
                    {items.length === 0 ? (
                        <motion.div variants={itemAnim} className="flex flex-col items-center justify-center py-32 bg-[#13100b] border border-white/5 rounded-[3rem] text-center px-6 shadow-3xl group relative overflow-hidden">
                            ...
                            <div className="w-24 h-24 rounded-full bg-white/5 flex items-center justify-center mb-8 relative">
                                <Clock className="w-12 h-12 text-gray-700 group-hover:text-amber-500 transition-colors duration-500" />
                                <div className="absolute inset-0 bg-amber-500/10 blur-[40px] opacity-0 group-hover:opacity-100 transition-opacity" />
                            </div>
                            ...
                            <Link to="/summarize" className="px-10 py-5 bg-amber-600 text-white rounded-full font-bold text-sm tracking-[0.2em] uppercase hover:bg-amber-700 transition-all shadow-2xl shadow-amber-600/20 active:scale-95 flex items-center gap-3">
                            ...
                            <AnimatePresence mode="popLayout">
                                {pagedItems.map((item) => (
                                    <motion.div
                                        ...
                                        className="group relative bg-[#171310] border border-white/5 rounded-[1.25rem] md:rounded-[2.5rem] p-4 md:p-6 lg:p-7 flex flex-col md:flex-row items-stretch md:items-center justify-between cursor-pointer hover:border-amber-500/50 transition-all hover:bg-white/[0.04] shadow-2xl overflow-hidden gap-3 md:gap-6"
                                    >
                                        ...
                                        <div className="flex items-center gap-4 md:gap-6 flex-1 min-w-0 relative z-10">
                                            <div className="w-10 h-10 md:w-16 md:h-16 rounded-xl md:rounded-2xl flex items-center justify-center border shrink-0 border-transparent transition-all duration-300 group-hover:scale-105 shadow-lg">
                                                ...
                                                {editingId === item.id ? (
                                                    <div className="flex items-center gap-2 w-full" onClick={(e) => e.stopPropagation()}>
                                                        <input
                                                            ...
                                                            className="flex-1 min-w-0 bg-black/60 border border-amber-500/50 rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none"
                                                            autoFocus
                                                        />
                                                    ...
                                                    <>
                                                        <h4 className="text-[13px] md:text-xl font-bold mb-1 truncate group-hover:text-amber-400 transition-colors pr-2 leading-tight">
                                                            {item.title}
                                                        </h4>
                                                        <div className="flex items-center gap-2 md:gap-3 text-[9px] md:text-[10px] font-bold uppercase tracking-wider text-gray-500">
                                                            ...
                                                            <span className="text-amber-500/70">{item.type}</span>
                                        ...
                                        {editingId !== item.id && (
                                            <div className="flex items-center justify-between md:justify-end gap-3 md:gap-4 relative z-10 pt-3 md:pt-0 border-t border-white/5 md:border-t-0">
                                                ...
                                                <div className="w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-gray-500 group-hover:bg-amber-600 group-hover:text-white group-hover:border-amber-500 transition-all shadow-lg ml-1">
