...
function Tooltip({ text, children, align = "center" }: { text: string; children: React.ReactNode; align?: "left" | "right" | "center" }) {
  ...
  return (
    <div className="group relative flex items-center justify-center">
      {children}
      <div className={cn(
        "absolute bottom-full mb-3 px-3 py-1.5 bg-[#171310] border border-white/10 rounded-lg text-[10px] text-white font-bold opacity-0 group-hover:opacity-100 transition-all pointer-events-none shadow-2xl z-[200] uppercase tracking-widest scale-95 group-hover:scale-100 origin-bottom min-w-[100px] max-w-[200px] text-center whitespace-nowrap",
        posClass
      )}>
        {text}
      </div>
...
function TooltipBelow({ text, children, align = "left" }: { text: string; children: React.ReactNode; align?: "left" | "right" | "center" }) {
  ...
  return (
    <div className="group relative flex items-center justify-center">
      {children}
      <div className={cn(
        "absolute top-full mt-3 px-3 py-1.5 bg-[#171310] border border-white/10 rounded-lg text-[10px] text-white font-bold opacity-0 group-hover:opacity-100 transition-all pointer-events-none shadow-2xl z-[200] uppercase tracking-widest scale-95 group-hover:scale-100 origin-top min-w-[100px] max-w-[200px] text-center whitespace-nowrap",
        posClass
      )}>
        {text}
      </div>
  ...
  return (
    ...
    >
      <button
        ...
        className="relative w-8 h-8 rounded-full border border-white/20 bg-[#15110c] hover:border-amber-500/40 transition-all"
      >
        <span
          className="absolute inset-[3px] rounded-full"
          style={{
            background: `conic-gradient(rgba(245,158,11,0.95) ${visualPct}%, rgba(255,255,255,0.14) ${visualPct}% 100%)`,
          }}
        />
        <span className="absolute inset-[7px] rounded-full bg-[#120e0a] border border-white/10 flex items-center justify-center text-[7px] font-bold text-amber-200">
          {Math.round(usedPct)}%
        </span>
      </button>
      ...
      <div
        className={cn(
          "absolute bottom-full right-0 mb-3 w-[246px] rounded-2xl border border-white/15 bg-[#1e1811]/95 backdrop-blur-xl px-3.5 py-3 shadow-2xl z-[260] transition-all duration-200",
      ...
      >
        ...
        <div className="mt-2 text-[11px] text-gray-300">Projected with input: <span className="text-amber-300 font-semibold">{Math.round(projectedPct)}%</span></div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 md:gap-3">
        <Sparkles className={cn("text-amber-400/90", compact ? "w-4 h-4" : "w-5 h-5")} />
        <span
          className={cn(
...
export function SummaryResult() {
  ...
  if (!state) {
    return (
      <div className="h-screen bg-[#0b0906] text-white flex items-center justify-center flex-col gap-4">
        ...
        <button
          ...
          className="px-8 py-4 rounded-full bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs uppercase tracking-widest transition-all shadow-lg active:scale-95"
        >
          Initialize Analysis
        </button>
      </div>
  ...
  return (
    <>
      ...
      <div className="w-full h-full flex flex-col bg-[#0b0906] text-white selection:bg-amber-500/30 font-sans relative pb-[72px] lg:pb-0 overflow-x-hidden">

        {/* Top Header (Sticky) */}
        <div className="sticky top-0 z-[150] shrink-0 bg-[#0b0906]/80 backdrop-blur-xl px-4 md:px-6 py-3 md:py-4 flex items-center justify-between border-b border-white/5">
          ...
          <div className="flex flex-col items-center">
            <h2 className="text-[11px] font-bold tracking-widest uppercase font-display text-amber-400 bg-amber-500/5 px-4 py-1.5 rounded-full border border-amber-500/10 shadow-[0_0_20px_rgba(245,158,11,0.1)]">ClipIQ Platform Console</h2>
          </div>
        ...
        >
          ...
          <div className="lg:hidden flex flex-col gap-4 px-4 py-4">
            ...
            >
              {validUrl ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-[#13100b]">
            ...
            <div className="space-y-2">
              ...
              {channel && (
                <span className="inline-block text-[9px] font-bold uppercase tracking-widest text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2.5 py-1 rounded-full">{channel}</span>
              )}
            </div>

            {/* Tab Switcher - Sticky at Top (Under Header) on Mobile */}
            <div className="sticky top-[58px] z-[120] -mx-4 px-4 py-3 bg-[#0b0906]/90 backdrop-blur-xl">
              <div className="flex gap-2 bg-white/[0.04] border border-white/10 p-1 rounded-2xl">
                <button
                  ...
                  className={cn(
                    ...
                    activeTab === "summary" ? "bg-amber-600 border border-amber-500/40 text-white shadow-[0_0_20px_rgba(217,119,6,0.2)]" : "text-gray-400 hover:text-white"
                  )}
                >
                  <Sparkles className={cn("w-3.5 h-3.5", activeTab === "summary" ? "text-amber-100" : "text-gray-500")} />
                  Summary
                </button>
                <button
                  ...
                  className={cn(
                    "flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all",
                    activeTab === "chat" ? "bg-amber-600 border border-amber-500/40 text-white shadow-[0_0_20px_rgba(217,119,6,0.2)]" : "text-gray-400 hover:text-white"
                  )}
                >
                  <MessageSquare className={cn("w-3.5 h-3.5", activeTab === "chat" ? "text-amber-100" : "text-gray-500")} />
                  Chat
                </button>
              </div>
            </div>
            ...
            {activeTab === "summary" ? (
              <div
                data-lenis-prevent
                className="bg-[#171310] rounded-2xl border border-white/5 p-4 h-[calc(100dvh-300px)] min-h-[460px] overflow-y-auto overscroll-y-contain custom-scrollbar"
                style={{ touchAction: "pan-y", WebkitOverflowScrolling: "touch" }}
              >
                <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/5">
                  <div>
                    <h3 className="text-sm font-serif italic text-amber-400 font-bold">Executive Intelligence Summary</h3>
                ...
                <div className="prose prose-invert prose-sm max-w-none pb-14 text-sans leading-relaxed text-[14px]">
                  <ReactMarkdown
                    components={{
                      ...
                      h3: ({ children }) => <h3 className="text-amber-300 font-bold text-base mt-4 mb-2 font-serif italic">{children}</h3>,
                      p: ({ children }) => {
                      ...
                      li: ({ children }) => {
                        ...
                        return (
                          <li className={cn("flex gap-3 items-start", isHeader && "mt-6 mb-3")}>
                            {!isHeader && <Sparkles className="w-3.5 h-3.5 text-amber-400 shrink-0 mt-1" />}
                            <span className={cn(
                      ...
                      a: ({ href, children }) => {
                        ...
                        if (timeMatch) {
                          ...
                          return (
                            <button onClick={() => handleSeek(seconds)} className="text-white font-mono font-bold inline-flex items-center gap-1.5 bg-amber-600/20 hover:bg-amber-600/40 px-2py-1 rounded-lg border border-amber-500/30 transition-all text-[11px] mx-1">
                              <Play className="w-3 h-3 fill-white" /><span>{displayTime}</span>
                            </button>
                          );
                        }
                        return <a href={href} target="_blank" rel="noreferrer" className="text-amber-400 font-bold underline decoration-amber-500/40 underline-offset-4 hover:text-white transition-all">{children}</a>;
                      },
                    }}
                  >
                    {summaryMarkdown}
              ...
              <div
                className="bg-[#100c08] rounded-3xl border border-white/5 flex flex-col overflow-hidden relative"
                style={{ height: isDesktop ? 'auto' : 'calc(100dvh - 340px)', minHeight: '440px', touchAction: "pan-y" }}
              >
                <div className="absolute top-0 right-0 w-32 h-32 bg-amber-600/[0.05] blur-[50px] pointer-events-none" />
                ...
                >
                  {chatMessages.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center py-10">
                      <div className="w-14 h-14 aspect-square rounded-2xl border border-amber-500/30 bg-amber-500/10 flex items-center justify-center mb-4 shadow-[0_0_40px_rgba(245,158,11,0.15)]"><MessageSquare className="w-7 h-7 text-amber-400" /></div>
                      <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-amber-300">ClipIQ Interface Ready</p><p className="text-[10px] text-gray-500 mt-2 max-w-[260px] leading-relaxed">Ask for timestamps, key moments, and scene-level insights.</p>
                      {starterQuestionsForVideo.length > 0 ? (
                        <div className="flex flex-wrap gap-2 mt-5 justify-center">
                          {starterQuestionsForVideo.map((q) => (
                            <button key={q} onClick={() => handleSendChat(q)}
                              className="px-4 py-2 bg-white/5 hover:bg-amber-600/20 border border-white/10 hover:border-amber-500 text-[10px] font-bold text-gray-300 hover:text-white rounded-full active:scale-95 transition-all disabled:opacity-50"
                      ...
                      {chatMessages.map((msg) => {
                        ...
                        return (
                          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} key={msg.id}
                            ...
                            {msg.role === "ai" && (
                              <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-amber-500/20 to-amber-500/5 border border-white/10 flex items-center justify-center mr-3 mt-1 shrink-0 shadow-lg">
                            ...
                            <div className={cn("flex flex-col min-w-0 gap-2", msg.role === "user" ? "max-w-[50%] items-end ml-auto" : "max-w-[92%] items-start")}>
                              <div className={cn(
                                ...
                                msg.role === "user"
                                  ? "bg-amber-600 text-white rounded-tr-none font-medium text-[14px]"
                                  : "bg-[#1a140f] border border-white/10 text-gray-200 rounded-tl-none text-[14px] leading-relaxed"
                              )}>
                                      ...
                                      li: ({ children }) => {
                                        ...
                                        return (
                                          <li className="flex gap-2.5 items-start mb-1 last:mb-0">
                                            <Sparkles className="w-3.5 h-3.5 text-amber-500/40 shrink-0 mt-1" />
                                      ...
                                      a: ({ href, children }) => {
                                        ...
                                        return (
                                          <span
                                            ...
                                            className="text-amber-400 font-bold underline decoration-amber-500/40 underline-offset-4 cursor-pointer"
                                          >
                                            {children}
                                          </span>
                                        );
                                        ...
                                        {timestamps.map((seconds, idx) => {
                                          ...
                                          return (
                                            <button
                                              ...
                                              className="inline-flex items-center gap-1 px-2.5 py-1 bg-amber-600/10 border border-amber-500/20 rounded-lg text-amber-400 font-mono text-[10px] font-bold hover:bg-amber-600/20 active:scale-95 transition-all"
                                            >
                            ...
                            {
                              msg.role === "user" && (
                                <div className="w-8 h-8 rounded-xl bg-amber-600/20 border border-amber-500/20 flex items-center justify-center ml-3 mt-1 shrink-0 shadow-lg">
                ...
                <div className="shrink-0 p-3 md:p-4 bg-transparent">
                  <div className="w-full mx-auto relative flex items-center gap-2 bg-white/[0.04] border border-white/10 rounded-[1.25rem] px-3 focus-within:border-amber-500/40 transition-all shadow-inner p-1.5">
                    <textarea
                      data-lenis-prevent
                      value={chatInput}
                      onChange={(e) => {
                    ...
                    <button onClick={() => handleSendChat()} disabled={!chatInput.trim() || isSending}
                      className="w-8 h-8 flex items-center justify-center bg-amber-600/80 text-white rounded-full disabled:opacity-20 active:scale-95 shrink-0 shadow-lg self-center">
          ...
          <div className="hidden lg:contents">
            ...
            <div className="lg:w-[32%] xl:w-[26%] flex flex-col overflow-y-auto overflow-x-hidden scrollbar-hide pb-6 shrink-0 lg:pr-2 overscroll-contain">
              ...
              >
                {validUrl ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-gray-700 gap-4 border border-white/5 bg-[#13100b]">
              ...
              <div className="space-y-4 md:space-y-6 shrink-0 pb-6 md:pb-8">
                ...
                <div className="flex flex-wrap items-center gap-2 md:gap-3">
                  <span className="px-3 py-1 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[9px] md:text-[10px] font-bold uppercase tracking-widest">{channel}</span>
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4 shrink-0">
                {[
                  ...
                  <div key={i} className="p-3 md:p-4 bg-white/[0.03] border border-white/5 rounded-2xl hover:bg-white/[0.05] transition-all group">
                    <div className="flex items-center gap-2 mb-1.5 md:mb-2 text-amber-500/50 group-hover:text-amber-400 transition-colors">
            ...
            <div className="flex-1 flex flex-col bg-[#100c08] rounded-[2rem] border border-white/5 shadow-[0_40px_100px_rgba(0,0,0,0.6)] relative min-h-0 overflow-hidden">
              <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-amber-600/[0.02] blur-[180px] pointer-events-none" />
              ...
              <div className="shrink-0 flex gap-2 p-2 md:p-3 bg-white/[0.03] backdrop-blur-2xl overflow-x-auto hide-scrollbar">
                ...
                >
                  <Sparkles className={cn("w-3.5 md:w-4 h-3.5 md:h-4", activeTab === "summary" ? "text-amber-400" : "text-gray-600")} />
                  Summary Map
                  {activeTab === "summary" && (
                    <motion.div
                      layoutId="activeTabDesktop"
                      initial={false}
                      className="absolute inset-0 bg-amber-500/10 border border-amber-500/30 rounded-xl md:rounded-2xl -z-10 shadow-[0_0_25px_rgba(245,158,11,0.15)]"
                    />
                  )}
                </button>
                <button
                ...
                >
                  <MessageSquare className={cn("w-3.5 md:w-4 h-3.5 md:h-4", activeTab === "chat" ? "text-amber-400" : "text-gray-600")} />
                  ClipIQ Chat
                  {activeTab === "chat" && (
                    <motion.div
                      layoutId="activeTabDesktop"
                  ...
                  {activeTab === "summary" ? (
                      ...
                      <div className="p-5 md:p-8 lg:p-14 w-full">
                        <div className="flex items-center justify-between mb-6 pb-5 border-b border-white/5">
                          <div className="flex flex-col gap-1.5">
                            <h3 className="text-lg md:text-2xl font-serif italic text-amber-400 font-bold tracking-tight">Executive Intelligence Summary</h3>
                        ...
                        <div className="prose prose-invert max-w-none font-sans pb-20">
                          <ReactMarkdown
                            components={{
                              ...
                              h3: ({ children }) => <h3 className="text-amber-300 font-bold text-base md:text-lg mt-5 mb-2 font-serif italic">{children}</h3>,
                              p: ({ children }) => {
                                const text = typeof children === "string" ? children : "";
                                if (/^key takeaways:?$/i.test(text.trim())) {
                              ...
                              li: ({ children }) => {
                                ...
                                return (
                                  <li className={cn("flex gap-3 items-start", isHeader ? "mt-4 mb-1 block w-full" : "mb-1")}>
                                    {!isHeader && <Sparkles className="w-3.5 h-3.5 text-amber-400 shrink-0 mt-1.5" />}
                                    <span className={cn(
                                ...
                                if (timeMatch) {
                                  ...
                                  return (
                                    <Tooltip text={`Jump to ${displayTime}`}>
                                      <button
                                        onClick={() => handleSeek(seconds)}
                                        className="text-white font-mono font-bold inline-flex items-center gap-2 bg-amber-600/20 hover:bg-amber-600/40 px-3 py-1.5 rounded-lg border border-amber-500/30 transition-all active:scale-90 mx-1 text-[12px]"
                                      >
                    ...
                    >
                      ...
                      >
                        {chatMessages.length === 0 ? (
                          <div className="h-full flex flex-col items-center justify-center text-center py-20 cursor-default">
                            <div className="w-20 h-20 aspect-square rounded-3xl border border-amber-500/30 bg-amber-500/10 flex items-center justify-center mb-6 shadow-[0_0_60px_rgba(245,158,11,0.2)]"><MessageSquare className="w-10 h-10 text-amber-400" /></div>
                            <h4 className="text-xl font-bold mb-3 font-display tracking-widest text-white uppercase">ClipIQ Interface Ready</h4>
                            <p className="text-[10px] max-w-[330px] font-bold uppercase tracking-[0.24em] text-amber-300/90 mb-3">Ask about specific timestamps, context, or hidden insights</p><p className="text-xs text-gray-500 mb-8">Try a precise query like: "What happens at 10:00?"</p>
                              ...
                              <div className="flex flex-wrap items-center justify-center gap-2 max-w-lg px-4">
                                {starterQuestionsForVideo.map((q) => (
                                  <button
                                    ...
                                    className="px-4 py-2 bg-white/5 hover:bg-amber-600/20 border border-white/10 hover:border-amber-500 text-[10px] md:text-xs font-bold text-gray-400 hover:text-white rounded-full transition-all active:scale-95 disabled:opacity-50"
                                    disabled={isSending}
                                  >
                                    {q}
                                  </button>
                          ...
                          <div className="space-y-10 pb-20">
                            {chatMessages.map((msg) => {
                              ...
                              return (
                                ...
                                >
                                  {msg.role === "user" ? (
                                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-amber-600/20 border border-amber-500/20 flex items-center justify-center ml-4 md:ml-5 mt-1 shrink-0 shadow-lg order-2 group-hover/msg:border-amber-500/40 transition-colors">
                                      <User className="w-5 h-5 md:w-6 md:h-6 text-amber-400" />
                                    </div>
                                  ) : (
                                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-gradient-to-br from-amber-500/20 to-amber-500/5 border border-white/10 flex items-center justify-center mr-4 md:mr-5 mt-1 shrink-0 shadow-lg group-hover/msg:border-amber-500/40 transition-colors">
                                  ...
                                  <div className={cn(
                                    ...
                                    <div className={cn(
                                      ...
                                      msg.role === "user"
                                        ? "bg-amber-600 text-white rounded-tr-none font-medium text-[13px] md:text-[14px]"
                                        : "bg-[#171310] border border-white/10 text-gray-300 rounded-tl-none text-[13px] md:text-[14px] leading-relaxed"
                                    )}>
                                      <div className="prose prose-invert max-w-none">
                                        <ReactMarkdown
                                          components={{
                                            ...
                                            h3: ({ children }) => <h3 className="text-amber-300 font-bold text-sm md:text-md mt-3 mb-1 font-serif italic">{children}</h3>,
                                            ...
                                            li: ({ children }) => {
                                              ...
                                              return (
                                                <li className="flex gap-3.5 items-start mb-1 last:mb-0 group/li">
                                                  <Sparkles className="w-3 md:w-3.5 h-3 md:h-3.5 text-amber-500/40 shrink-0 mt-1.5 transition-all group-hover/li:text-amber-400" />
                                            ...
                                            a: ({ href, children }) => {
                                              ...
                                              return (
                                                <span
                                                  onClick={() => timeMatch && handleSeek(parseInt(timeMatch[1], 10))}
                                                  className="text-amber-400 font-bold underline underline-offset-8 hover:text-white transition-all cursor-pointer"
                                                >
                                                  {children}
                                                </span>
                                              );
                                              ...
                                              {timestamps.map((seconds, idx) => {
                                                ...
                                                return (
                                                  <Tooltip key={idx} text={`Seek to ${timeStr}`} align="left">
                                                    <button
                                                      onClick={() => handleSeek(seconds)}
                                                      className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-600/10 border border-amber-500/30 rounded-lg text-amber-400 font-mono text-[10px] font-bold hover:bg-amber-600/20 active:scale-95 transition-all shadow-sm"
                                                    >
                                                      <Play className="w-2.5 h-2.5 fill-current" />
                                                      {timeStr}
                                                    </button>
                      ...
                      <div className="shrink-0 px-6 pb-4 md:px-8 md:pb-6 lg:px-10 lg:pb-8 relative z-10 w-full mt-auto">
                        <div className="max-w-4xl mx-auto relative flex items-center gap-2 bg-white/[0.03] border border-white/10 rounded-[1.5rem] transition-all p-2 md:p-2.5 focus-within:border-amber-500/40 focus-within:bg-white/[0.05]">
                          <textarea
                            data-lenis-prevent
                            value={chatInput}
                            onChange={(e) => {
                          ...
                          <button
                            ...
                            className="w-8 h-8 flex items-center justify-center bg-amber-600/80 text-white rounded-full disabled:opacity-20 hover:bg-amber-600 transition-all shrink-0 shadow-lg self-center"
                          >
