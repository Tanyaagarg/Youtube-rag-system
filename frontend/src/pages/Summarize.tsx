...
function ProcessingTerminal({ status }: { status: string }) {
  ...
  const steps = [
    { text: "Initializing RAG pipeline...", color: "text-amber-400" },
    ...
    { text: "Extracting key insights...", color: "text-amber-400" },
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-2xl mx-auto bg-[#13100b] border border-white/10 rounded-3xl p-8 relative shadow-2xl mt-12 overflow-hidden"
    >
...
export function Summarize() {
  ...
  return (
    <div className="w-full bg-[#0b0906] selection:bg-amber-500/30 relative pb-[72px] md:pb-0 overflow-hidden">
      ...
      <div className="flex flex-col items-center p-4 md:p-6 lg:p-10 pt-4 md:pt-6 pb-20 md:pb-0">
        ...
        >
          {!isAnalyzing ? (
            <>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[10px] font-bold uppercase tracking-widest mx-auto">
              ...
              <div className="space-y-4 md:space-y-6">
                <h1 className="text-4xl md:text-8xl font-bold tracking-tight font-serif italic leading-[1.1] md:leading-[0.9] mb-4 text-white">
                  Analyze <br />
                  <span className="text-amber-500">any video.</span>
                </h1>
              ...
              <form onSubmit={handleSummarize} className="relative max-w-2xl mx-auto mt-8 md:mt-12 group px-4 md:px-0">
                <div className="relative flex flex-col sm:flex-row items-center bg-white/[0.03] border border-white/10 rounded-full p-2 backdrop-blur-xl transition-all focus-within:ring-4 focus-within:ring-amber-500/10 focus-within:border-amber-500/30 gap-2">
                  <div className="flex items-center flex-1 px-2">
                    <div className="pl-4 pr-2 text-white/40 group-focus-within:text-amber-500 transition-colors shrink-0 relative z-10">
                  ...
                  >
                    <div className="relative z-10 flex items-center justify-center gap-2 transition-colors duration-500">
                      <div className={cn(
                        "w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center transition-all duration-500 bg-black/[0.03] group-hover:bg-amber-600",
                        !url || isAnalyzing ? "bg-white/5" : ""
                      )}>
                        {isAnalyzing ? (
                          <Loader2 className="w-3.5 h-3.5 animate-spin text-black/50" />
                        ) : (
                          <Sparkles className={cn("w-3.5 h-3.5 transition-colors", url && !isAnalyzing ? "text-amber-600 group-hover:text-white" : "text-gray-800")} />
                        )}
                      </div>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-r from-amber-400/0 via-amber-400/10 to-orange-400/0 opacity-0 group-hover:opacity-100 translate-x-[-100%] group-hover:translate-x-[100%] transition-all duration-[1s] ease-in-out pointer-events-none" />
                  </button>
                </div>
              </form>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-12 max-w-3xl mx-auto">
                {[
                  ...
                  <div
                    key={t.label}
                    className="p-4 bg-[#13100b] border border-white/5 rounded-2xl text-left hover:border-amber-500/30 transition-all cursor-default group"
                  >
                    <div className="text-[11px] font-bold text-white uppercase tracking-wider mb-1 group-hover:text-amber-400 transition-colors">{t.label}</div>
