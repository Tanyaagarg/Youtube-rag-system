...
def _build_summary_source_text(chunks, max_chars: int = 45000) -> str:
    ...
    Kept well under the smallest free-tier OpenRouter context window this
    app targets (~65k tokens, input+output combined). Measured against real
    transcripts this content tokenizes far denser than the ~4-chars/token
    rule of thumb (observed ~1.2 chars/token for one video), so 45,000
    chars still leaves comfortable headroom for the prompt template, JSON
    format instructions, and the completion itself.
    """
    total_text = " ".join([c.page_content for c in chunks])
    if len(total_text) <= max_chars:
        return total_text

    # Sample evenly across the video (not just the first N chars) so the
    # summary still covers beginning/middle/end, but enforce max_chars as a
    # true hard cap: chunk sizes vary a lot (overlap splitting can leave
    # short remainder chunks next to full-size ones), so a fixed stride over
    # the chunk list does NOT bound the joined length -- it can easily land
    # on a run of oversized chunks and blow past max_chars anyway.
    step = max(1, len(total_text) // max_chars + 1)
    sampled = chunks[::step] or chunks
    pieces: list[str] = []
    used = 0
    for c in sampled:
        ...
        pieces.append(piece)
        used += len(piece) + 1  # +1 for the join separator
    return " ".join(pieces)
